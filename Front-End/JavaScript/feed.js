...
import { fetchCurrentProfile } from "./shared/services/profile-service.js";
import {
  fetchFeed,
  fetchPostById,
  likePost,
...
if (shellOk) {
              ...
              ${
                ehDono
                  ? `<div class="comentario-acoes">
                      <button class="btn-icon" type="button" data-action="abrir-editar-comentario" data-post-id="${post.id}" data-comment-id="${comentario.id}"><i data-lucide="pencil"></i></button>
                      <button class="btn-icon" type="button" data-action="abrir-excluir-comentario" data-post-id="${post.id}" data-comment-id="${comentario.id}"><i data-lucide="trash-2"></i></button>
                    </div>`
                  : ""
              }
            </div>
  ...
  function criarPostHTML(post) {
    ...
    return `
      <article class="card" data-post-id="${post.id}">
        <div class="post-topo">
          <img class="avatar" src="${escaparHTML(post.foto)}" alt="">
          <div class="info">
          ...
          ${
            ehMeuPost
              ? `<div class="post-topo-acoes">
                  <button class="btn-icon" type="button" data-action="abrir-editar-post" data-post-id="${post.id}"><i data-lucide="pencil"></i></button>
                  <button class="btn-icon" type="button" data-action="abrir-excluir-post" data-post-id="${post.id}"><i data-lucide="trash-2"></i></button>
                </div>`
              : ""
          }
        </div>
        ...
        <div class="post-acoes">
          ...
          <div class="comment-badge"><i data-lucide="message-circle"></i><span>${post.comentarios.length}</span></div>
        </div>
        <div class="comentarios">${post.comentarios.map((c) => criarComentarioHTML(post, c)).join("")}</div>
        <div class="add-comentario">
          ...
          <button class="btn-comentar" type="button" data-action="criar-comentario" data-post-id="${post.id}"><i data-lucide="send"></i></button>
        </div>
      </article>
    `;
  }
  ...
  async function alternarLike(postId, likeButton) {
    ...
    try {
      ...
      atualizarPostNaLista({ id: postId, liked: atualizado.liked, likes: atualizado.likes });
  ...
  async function salvarEdicaoPost() {
    ...
    try {
      const resposta = await updatePost(postEmEdicao.id, novoTexto, postEmEdicao.imagem || "");
  ...
  feedEl?.addEventListener("click", async (event) => {
    ...
    if (action === "abrir-editar-comentario") {
      ...
      editCommentInput.value = c?.texto || "";
  ...
  async function initFeed() {
    try {
      ...
      postsCarregados = await fetchFeed();
